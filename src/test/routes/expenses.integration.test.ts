...
beforeEach(async () => {
  const category = await request(app).post("/categories").send({ name: "Food" });
...
describe("expenses routes", () => {
  ...
  it("POST /expenses fails when a required field is missing", async () => {
    ...
    const response = await request(app).post("/expenses").send(incomplete);

    expect(response.status).toBe(400);
  });

  it("POST /expenses fails with an unsupported currency", async () => {
    const response = await request(app).post("/expenses").send({ ...validExpense(), currency: "XYZ" });

    expect(response.status).toBe(400);
  });

  it("POST /expenses fails with a categoryId that doesn't exist", async () => {
    const response = await request(app).post("/expenses").send({ ...validExpense(), categoryId: 999999 });
  ...
  it("GET /expenses/:id returns the expense", async () => {
    ...
    const response = await request(app).get(`/expenses/${created.body.id}`);
  ...
  it("PATCH /expenses/:id updates categoryId and date", async () => {
    const created = await request(app).post("/expenses").send(validExpense());
    const newCategory = await request(app).post("/categories").send({ name: "Travel" });
  ...
  it("PATCH /expenses/:id returns 404 for a nonexistent id", async () => {
    const response = await request(app).patch("/expenses/999999").send({ amount: 2000 });
  ...
  it("DELETE /expenses/:id removes the expense", async () => {
    ...
    const deleteResponse = await request(app).delete(`/expenses/${created.body.id}`);
    expect(deleteResponse.status).toBe(200);

    const getResponse = await request(app).get(`/expenses/${created.body.id}`);
  ...
  it("DELETE /expenses/:id returns 404 on a second delete", async () => {
    ...
    const secondDelete = await request(app).delete(`/expenses/${created.body.id}`);
